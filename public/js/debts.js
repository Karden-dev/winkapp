...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const fetchShops = async () => {
        try {
            const response = await axiosInstance.get('/shops?status=actif');
    ...
    debtForm.addEventListener('submit', async (e) => {
        ...
        try {
            if (debtIdInput.value) {
                // Modification
                await axiosInstance.put(`/debts/${debtIdInput.value}`, debtData);
                showNotification("Créance modifiée avec succès !");
            } else {
                // Création
                await axiosInstance.post('/debts', debtData);
    ...
    debtsTableBody.addEventListener('click', async (e) => {
        ...
        if (target.classList.contains('edit-btn')) {
            try {
                const response = await axiosInstance.get(`/debts/${debtId}`);
            ...
            if (confirm("Êtes-vous sûr de vouloir supprimer cette créance ?")) {
             ...
             if (confirm("Confirmer le règlement de cette créance ?")) {
                try {
                    await axiosInstance.put(`/debts/${debtId}/settle`, { userId: CURRENT_USER_ID });
