...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const fetchRemittances = async () => {
        try {
            ...
            const response = await axiosInstance.get('/remittances', { params });
    ...
    remittanceTableBody.addEventListener('click', async (e) => {
        ...
        if (target.classList.contains('edit-payment-btn')) {
            ...
            const shop = (await axiosInstance.get(`/shops/${shopId}`)).data;
            if (confirm(`Confirmer le versement complet de ${amount.toLocaleString('fr-FR')} FCFA à ${shop.name} ?`)) {
                try {
                    await axiosInstance.post('/remittances/record', {
                        shopId: shopId,
                        amount: amount,
            ...
            try {
                const response = await axiosInstance.get(`/remittances/details/${shopId}`);
    ...
    editPaymentForm.addEventListener('submit', async (e) => {
        ...
        try {
            await axiosInstance.put(`/remittances/shop-details/${shopId}`, paymentData);
    ...
    bulkPayBtn.addEventListener('click', async () => {
        const response = await axiosInstance.get('/remittances');
        const remittances = response.data.remittances;
        ...
        if (confirm(`Confirmer le versement de ${pendingRemittances.length} marchands ?`)) {
            try {
                const bulkPayPromises = pendingRemittances.map(rem =>
                    axiosInstance.post('/remittances/record', {
                        shopId: rem.shop_id,
    ...
    exportPdfBtn.addEventListener('click', async () => {
        try {
            const response = await axiosInstance.get('/remittances/export-pdf', { responseType: 'blob' });
