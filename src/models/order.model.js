...
const update = async (orderId, orderData, userId) => {
    const connection = await dbConnection.getConnection();
    try {
        await connection.beginTransaction();
        ...
        const params = [];
        const [previousOrder] = await connection.execute('SELECT expedition_fee FROM orders WHERE id = ?', [orderId]);
        const previousExpeditionFee = previousOrder[0]?.expedition_fee;
        
        const validFields = ['shop_id', 'customer_name', 'customer_phone', 'delivery_location', 'article_amount', 'delivery_fee', 'deliveryman_id', 'created_at', 'expedition_fee'];
        ...
        if (fieldsToUpdate.length > 0) {
            fieldsToUpdate.push('updated_by = ?', 'updated_at = NOW()');
            params.push(userId);
        ...
        if (newExpeditionFee !== previousExpeditionFee) {
            const comment = `Frais d'expédition pour la commande n°${orderId}`;
            await connection.execute('DELETE FROM debts WHERE order_id = ? AND type = "expedition"', [orderId]);
...
const updateStatus = async (orderId, status, amountReceived = null, paymentStatus = null, userId) => {
    const connection = await dbConnection.getConnection();
    try {
        await connection.beginTransaction();
        ...
        const [order] = await connection.execute('SELECT deliveryman_id, article_amount, amount_received FROM orders WHERE id = ?', [orderId]);
        const deliverymanId = order[0]?.deliveryman_id;

        if (cashService && deliverymanId) {
            // Logique anti-doublon : supprimer tout versement en attente pour cette commande
            const commentLike = `%commande n°${orderId}`;
            await connection.execute(`DELETE FROM cash_transactions WHERE comment LIKE ? AND status = 'pending'`, [commentLike]);

            // Si la livraison génère un encaissement, créer un nouveau versement en attente
            if ((status === 'delivered' && paymentStatus === 'cash') || (status === 'failed_delivery' && amountReceived > 0)) {
                const amount = (status === 'delivered') ? parseFloat(order[0].article_amount) : parseFloat(amountReceived);
                const comment = `Versement en attente pour la commande n°${orderId}`;
                await cashService.createRemittanceTransaction(deliverymanId, amount, comment);
            }
        }
...
const assignDeliveryman = async (orderId, deliverymanId, userId) => {
    const connection = await dbConnection.getConnection();
    try {
        await connection.beginTransaction();
        ...
        await connection.execute(
            'UPDATE orders SET deliveryman_id = ?, status = ?, payment_status = ?, updated_by = ?, updated_at = NOW() WHERE id = ?',
            [deliverymanId, 'in_progress', 'pending', userId, orderId]
        );

        await connection.execute('INSERT INTO order_history (order_id, action, user_id) VALUES (?, ?, ?)', [orderId, `Commande assignée au livreur : ${deliverymanName}`, userId]);

        // Logique anti-doublon : si un versement en attente existait pour un autre livreur, le supprimer
        if (cashService) {
            const commentLike = `%commande n°${orderId}`;
            await connection.execute(`DELETE FROM cash_transactions WHERE comment LIKE ? AND status = 'pending'`, [commentLike]);
        }
...
const findReportsByDateRange = async (startDate, endDate) => {
    const connection = await dbConnection.getConnection();
    try {
        const query = `
            SELECT
